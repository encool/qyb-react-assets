...
const repo_url = "https://github.com/encool/qyb-umi-assets"
...
function parseJSON(root) {
  ...
  const list = dirs.reduce((memo = [], dir) => {
    ...
    const url = `${repo_url}/tree/master/${type}/${dir}`;
    const img = `${repo_url}/blob/master/${type}/${dir}/snapshot.png?raw=true`;
    const features = [];
